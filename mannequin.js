...
class ParametricShape extends THREE.Group {
	...
	addSphere (r, y, x = 0, z = 0) {
		let s = Mannequin.sphereTemplate.clone ();
...
class HeadShape extends ParametricShape {
	constructor (feminine, params) {
		super (Mannequin.texHead, Mannequin.colors[0], (u, v, target) => {
			let r = Mannequin.cossers (u, v, [ [ 0.4, 0.9, 0, 1, -3 ],
				[ 0, 1, 0, 0.1, 3 ],
				[ 0, 1, 0.9, 1, 3 ],
			...
			u = 360 * u;
			v = 180 * v - 90;
			let k = (1 + (feminine ? 1 : 2) * sin (u) * cos (v)) / 4;
...
class ShoeShape extends THREE.Group {
	constructor (feminine, params) {
		...
		this.add (new ParametricShape (Mannequin.texLimb, Mannequin.colors[1], (u, v, target) => {
			let r = Mannequin.cossers (u, v, [ [ 0.6, 1.1, 0.05, 0.95, 1 ], [ 0.6, 0.68, 0.35, 0.65, feminine ? 1.2 : 1000 ] ]);

			u = 360 * u;
			v = 180 * v - 90;
			target.set (
				(3 * r - 2) * params[0] * (cos (u) * cos (v) + (feminine ? sin (u + 180) ** 2 * cos (v) - 1 : 0)) - (feminine ? 0 : 2),
		...
		if (feminine) {
			this.add (new ParametricShape (Mannequin.texLimb, Mannequin.colors[4], (u, v, target) => {
				let r = Mannequin.cossers (u, v, [ [ 0.6, 1.1, 0.05, 0.95, 1 / 2 ] ]);
...
class PelvisShape extends ParametricShape {
	constructor (feminine, params) {
		super (Mannequin.texLimb, Mannequin.colors[2], (u, v, target) => {
			let r = Mannequin.cossers (u, v, [ [ 0.6, 0.95, 0, 1, 4 ], [ 0.7, 1.0, 0.475, 0.525, -13 ], [ -0.2, 0.3, 0, 0.3, -4 ], [ -0.2, 0.3, -0.3, 0, -4 ] ]);

			u = 360 * u - 90;
			v = 180 * v - 90;
			target.set (
				-1.5 + r * params[0] * cos (u) * cos (v) ** 0.6,
				r * params[1] * sin (u) * cos (v) ** 0.6,
				r * params[2] * sin (v)
			);
		}, 20, 10);
...
class LimbShape extends ParametricShape {
	constructor (feminine, params, nU = 24, nV = 12) {
		...
		super (Mannequin.texLimb, Mannequin.colors[4], (u, v, target) => {
			v = 360 * v;
			let r = offset + scale * cos (alpha + dAlpha * u);

			target.set (x * r * cos (v) / 2, y * u, z * r * sin (v) / 2);
			let w = new THREE.Vector3 (x * cos (v) * cos (170 * u - 85) / 2,
				y * (1 / 2 + sin (180 * u - 90) / 2),
				z * sin (v) * cos (180 * u - 90) / 2);

			target = target.lerp (w, Math.abs (2 * u - 1) ** 16);
		}, nU, nV);
		this.children[0].position.set (0, -y / 2, 0);

		if (rad) {
			this.addSphere (rad ? rad : z / 2, -y / 2);
		}
...
class TorsoShape extends ParametricShape {
	constructor (feminine, params) {
		...
		super (Mannequin.texLimb, Mannequin.colors[5], (u, v, target) => {
			let r = offset + scale * cos (alpha + dAlpha * u);

			if (feminine) {
				r += Mannequin.cossers (u, v, [ [ 0.35, 0.85, 0.7, 0.95, 2 ], [ 0.35, 0.85, 0.55, 0.8, 2 ] ]) - 1;
			}

			v = 360 * v + 90;
			let x1 = x * (0.3 + r) * cos (v) / 2,
				y1 = y * u,
				z1 = z * r * sin (v) / 2;
			let x2 = x * cos (v) * cos (180 * u - 90) / 2,
				y2 = y * (1 / 2 + sin (180 * u - 90) / 2),
				z2 = z * sin (v) * cos (180 * u - 90) / 2;
			let k = Math.abs (2 * u - 1) ** 16,
				kx = Math.abs (2 * u - 1) ** 2;
...
class Joint extends THREE.Group {
	...
	getBumper (x, y, z) {
		let bumper = new THREE.Vector3 (x, y, z);
	...
	select (state) {
		this.traverse ((o) => {
			if (o.material && o.material.emissive) {
				o.material.emissive.setRGB (0, state ? -1 : 0, state ? -0.4 : 0);
			}
		});
	} // Joint.select
...
class Head extends Joint {
	...
	set nod (angle)		{
		this.z = -angle / 2; this.parentJoint.z = -angle / 2;
	}
	...
	set tilt (angle)		{
		this.x = -angle / 2; this.parentJoint.x = -angle / 2;
	}
	...
	set turn (angle)		{
		this.y = angle / 2; this.parentJoint.y = angle / 2;
	}
...
class Arm extends Joint {
	...
	biologicallyImpossibleLevel () {
		...
		let p = this.getBumper (0, 15, -0 * 5 * this.leftOrRight);

		if (p.z * this.leftOrRight < -3) {
			result += -3 - p.z * this.leftOrRight;
		}
		...
		this.rotation.reorder ('ZXY');
		let r = this.rotation.y * 180 / Math.PI;
		let min = -90;
		let max = 90;
