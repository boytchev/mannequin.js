...
function drawFrame()
{if(animate)animate(100*clock.getElapsedTime());renderer.render(scene,camera);}
function rad(x){return x*Math.PI/180;}
function sin(x){return Math.sin(rad(x));}
function cos(x){return Math.cos(rad(x));}
const LEFT=-1;const RIGHT=1;var colors=['antiquewhite','gray','antiquewhite','burlywood','antiquewhite','bisque'];var texHead=new THREE.TextureLoader().load("data:image/png;base64,");var texLimb=new THREE.TextureLoader().load("data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAEAAAABAAQMAAACQp+OdAAAABlBMVEX////Ly8vsgL9iAAAAHElEQVQoz2OgEPyHAjgDjxoKGWTaRRkYDR/8AAAU9d8hJ6+ZxgAAAABJRU5ErkJggg==");var sphere=new THREE.Mesh(new THREE.SphereBufferGeometry(1,16,8),new THREE.MeshPhongMaterial({color:colors[3],shininess:1}));sphere.castShadow=true;sphere.receiveShadow=true;function cossers(u,v,params)
...
addSphere(r,y)
{var s=sphere.clone();s.scale.set(r,r,r);s.position.set(0,y,0);this.add(s);}}
...
{constructor(feminine,params)
{super();this.add(new ParametricShape(texLimb,colors[1],function(u,v,target)
...
{var x=params[0],y=params[1],z=params[2],alpha=params[3],dAlpha=params[4],offset=params[5],scale=params[6],rad=params[7];super(texLimb,colors[4],function(u,v,target)
...
{constructor(feminine,params)
{var x=params[0],y=params[1],z=params[2],alpha=params[3],dAlpha=params[4],offset=params[5],scale=params[6];super(texLimb,colors[5],function(u,v,target)
...
{constructor(parentJoint)
{super(parentJoint,[1.2,1.5,3.5,0,45,0.3,0.4,0.2]);this.sub_finger=new Phalange(this,[1.2,1,3.5,45,45,0.3,0.4,0.2]);}
_turn()
{this.turnJoint(this.tiltAngle/4,-this.turnAngle/4,this.bendAngle);this.sub_finger.turnJoint(this.tiltAngle/3,-this.turnAngle/3,this.bendAngle);}}
