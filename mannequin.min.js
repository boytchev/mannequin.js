...
const LEFT=-1;const RIGHT=1;function cossers(u,v,params)
...
{var r=cossers(u,v,[[0.6,1.1,0.05,0.95,1],[0.6,0.68,0.35,0.65,feminine?1.2:1000]]);u=360*u;v=180*v-90;target.set((3*r-2)*params[0]*(cos(u)*cos(v)+(feminine?(Math.pow(sin(u+180),2)*cos(v)-1):0))-(feminine?0:2),params[1]*sin(u)*cos(v)+2,params[2]*sin(v));},24,12));if(feminine)
{this.add(new ParametricShape(Mannequin.texLimb,Mannequin.colors[4],function(u,v,target)
{var r=cossers(u,v,[[0.6,1.1,0.05,0.95,1/2]]);u=360*u;v=180*v-90;target.set(0.3*(3*r-2)*params[0]*(cos(u)*cos(v)),0.8*params[1]*sin(u)*cos(v)+2,0.6*params[2]*sin(v));},12,12));this.children[0].rotation.set(0,0,0.4);this.children[1].rotation.set(0,0,0.4);}}}
class PelvisShape extends ParametricShape
{constructor(feminine,params)
{super(Mannequin.texLimb,Mannequin.colors[2],function(u,v,target)
{var r=cossers(u,v,[[0.6,0.95,0,1,4],[0.7,1.0,0.475,0.525,-13],[0.0,0.3,0.3,0.9,feminine?1000:5],[-0.2,0.3,0,0.3,-4],[-0.2,0.3,-0.3,0,-4]]);u=360*u-90;v=180*v-90;target.set(-1.5+r*params[0]*cos(u)*Math.pow(cos(v),0.6),r*params[1]*sin(u)*Math.pow(cos(v),0.6),r*params[2]*sin(v));},20,10);}}
...
{var r=offset+scale*cos(alpha+dAlpha*u);if(feminine)r+=cossers(u,v,[[0.35,0.85,0.7,0.95,2],[0.35,0.85,0.55,0.8,2]])-1;v=360*v+90;var x1=x*(0.3+r)*cos(v)/2;var y1=y*u;var z1=z*r*sin(v)/2;var x2=x*cos(v)*cos(180*u-90)/2;var y2=y*(1/2+sin(180*u-90)/2);var z2=z*sin(v)*cos(180*u-90)/2;var k=Math.pow(Math.abs(2*u-1),16);var kx=Math.pow(Math.abs(2*u-1),2);if(x2<0)kx=k;target.set(x1*(1-kx)+kx*x2,y1*(1-k)+k*y2,z1*(1-k)+k*z2);},30,20);this.children[0].position.set(0,-y/2,0);this.addSphere(2,-y/2);}}
...
direct(x,y,z)
{this.tiltAngle=x;this.turnAngle=y;this.bendAngle=z;this._turn();}
bend(angle)
{this.bendAngle=angle;this._turn();}
turn(angle,leftOrRight=LEFT)
{this.turnAngle=leftOrRight*angle;this._turn();}
tilt(angle,leftOrRight=LEFT)
{this.tiltAngle=leftOrRight*angle;this._turn();}
_turn()
...
{var joint=this.userJoint.children[0];if(!(color instanceof THREE.Color))
color=new THREE.Color(color);if(color)joint.children[0].material.color=color;if(joint.children.length>1)
{if(!(secondaryColor instanceof THREE.Color))
secondaryColor=new THREE.Color(secondaryColor);joint.children[1].material.color=secondaryColor;}}}
...
{this.bendAngle=angle;this._turn();}
_turn()
{this.turnJoint(this.tiltAngle/2,-this.turnAngle/2,-this.bendAngle/2);this.parentJoint.turnJoint(this.tiltAngle/2,-this.turnAngle/2,-this.bendAngle/2);}}
...
_turn()
{this.turnJoint(this.tiltAngle/4,-this.turnAngle/4,this.bendAngle);this.tips.turnJoint(this.tiltAngle/3,-this.turnAngle/3,this.bendAngle);}}
